...
#ifndef SHERPA_DEPLOY_OPENVINO_STREAM_H_
#define SHERPA_DEPLOY_OPENVINO_STREAM_H_
...
class Stream {
 public:
  explicit Stream(const SherpaDeploy::FeatureExtractorConfig &config = {},
                  SherpaDeploy::ContextGraphPtr context_graph = nullptr);
  ~Stream();
   ...
   * @return Return a tuple (features_vector, feature_dim).
  ...
  void SetStates(const std::vector<ov::Tensor> &states);
  std::vector<ov::Tensor> &GetStates();
  /**
   * Get the context graph corresponding to this stream.
   *
   * @return Return the context graph for this stream.
   */
  const SherpaDeploy::ContextGraphPtr &GetContextGraph() const;
...
#endif  // SHERPA_DEPLOY_OPENVINO_STREAM_H_
