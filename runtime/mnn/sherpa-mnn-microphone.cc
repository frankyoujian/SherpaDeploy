...
#include "runtime/mnn/c-api/c-api.h"
...
static int32_t RecordCallback(const void *input_buffer,
                              ...
                              void *user_data) {
  auto s = reinterpret_cast<SherpaDeployMnnStream*>(user_data);
...
Usage:
  ./bin/SherpaDeploy-mnn-microphone \
    /path/to/encoder.mnn \
    /path/to/decoder.mnn \
    /path/to/joiner.mnn \
    /path/to/tokens.txt \
    [num_threads] \
    [decode_method, can be greedy_search/modified_beam_search] \
    [hotwords_file] \
    [hotwords_score] \
    [bpe_vocab]
  ...
  SherpaDeployMnnRecognizerConfig config;
  ...
  config.model_config.forward_type = forward;
  config.model_config.backend_precision_mode = precision;
  config.model_config.backend_power_mode = power;
  config.model_config.backend_memory_mode = memory;

  const float expected_sampling_rate = 16000;
  if (argc >= 7) {
    std::string method = argv[6];
    if (method == "greedy_search" || method == "modified_beam_search") {
      config.decoder_config.decoding_method = method.c_str();
    }
  ...
  if (argc >= 8) {
    config.hotwords_file = argv[7];
  }

  if (argc >= 9) {
    config.hotwords_score = atof(argv[8]);
  }

  if (argc >= 10) {
    config.model_config.modeling_unit = "bpe";
    config.model_config.bpe_vocab = argv[9];
  }
  ...
  SherpaDeployMnnRecognizer* recognizer = CreateRecognizer(&config);
  SherpaDeployMnnStream* s = CreateStream(recognizer);
