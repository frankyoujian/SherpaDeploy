...
**Note**: We only support 16000Hz wav files.
  ...
  if (!SherpaDeploy::FileExists(input_wave)) {
    fprintf(stderr, "%s %d: %s", __FILE__, static_cast<int32_t>(__LINE__),
            usage.c_str());
    return -1;
  }
  ...
  std::vector<float> samples =
      SherpaDeploy::ReadWave(input_wave, config.sample_rate, &is_ok);
  if (!is_ok) {
  ...
  is_ok = SherpaDeploy::WriteWave(out_wave, config.sample_rate,
