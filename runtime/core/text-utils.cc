...
static std::vector<std::string> MergeCharactersIntoWords(
  ...
  while (i < n) {
    ...
    fprintf(stderr,"Ignore %s", w.c_str());
    ++i;
  }
...
std::vector<std::string> SplitUtf8(const std::string &text) {
  ...
  while (start < end) {
    ...
    if (num_bytes == 0) {
      ...
      fprintf(stderr,"Invalid byte at position: %d",
...
std::string RemoveInvalidUtf8Sequences(const std::string &text,
  ...
  while (i < n) {
    ...
    if (show_debug_msg) {
      fprintf(stderr,"Ignore invalid utf8 sequence at pos: %d, value: %02x",
                       i, p[i]);
    }
...
std::string Gb2312ToUtf8(const std::string &text) {
  ...
  fprintf(stderr,"num of wchars: %d", num_wchars);
  if (num_wchars == 0) {
    return {};
  }
