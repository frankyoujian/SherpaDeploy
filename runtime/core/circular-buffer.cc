...
CircularBuffer::CircularBuffer(int32_t capacity) {
  if (capacity <= 0) {
    fprintf(stderr, "Please specify a positive capacity. Given: %d\n", capacity);
    exit(-1);
  }
...
void CircularBuffer::Resize(int32_t new_capacity) {
  ...
  if (new_capacity <= capacity) {
    fprintf(stderr, "new_capacity (%d) <= original capacity (%d). Skip it.",
...
std::vector<float> CircularBuffer::Get(int32_t start_index, int32_t n) const {
  if (start_index < head_ || start_index >= tail_) {
    fprintf(stderr, "Invalid start_index: %d. head_: %d, tail_: %d", start_index,
              head_, tail_);
    return {};
  }
  ...
  int32_t size = Size();
  if (n < 0 || n > size) {
    fprintf(stderr, "Invalid n: %d. size: %d", n, size);
    return {};
  }
  ...
  if (start_index - head_ + n > size) {
    fprintf(stderr, "Invalid start_index: %d and n: %d. head_: %d, size: %d",
