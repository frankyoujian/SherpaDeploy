...
static bool EncodeBase(const std::vector<std::string> &lines,
  ...
  for (const auto &line : lines) {
    ...
    while (iss >> word) {
      // replace ▁ with " ", since the symbol-table also replaces it.
      // This is different from the implementation of onnxruntime. In onnxruntime, symbol-table does not make this replacement
      if (word.size() >= 3) {
